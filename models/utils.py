...
def get_rotation(vec1, vec2):
    ...
    s_sq = torch.sum(v**2, dim=1, keepdim=True) + 1e-8
...
def generate_sphere_anchors(n_anchors: int = 64, device=None) -> torch.Tensor:
    """
    Generate uniformly distributed anchor points on the unit sphere
    using the Fibonacci sphere lattice method.

    This produces near-optimal uniform sampling on S^2, better than
    naive lat-lon grids which cluster at the poles.
    ...
    Returns:
        anchors: [n_anchors, 3] unit vectors on the sphere
    """
    # Fibonacci sphere algorithm
    indices = torch.arange(0, n_anchors, dtype=torch.float32, device=device)
    phi = math.pi * (3.0 - math.sqrt(5.0))  # golden angle in radians

    y = 1.0 - (indices / (n_anchors - 1)) * 2.0  # y goes from 1 to -1
    radius = torch.sqrt(1.0 - y * y)  # radius at y
    ...
    anchors = torch.stack([x, y, z], dim=-1)  # [n_anchors, 3]
